...
def parse_line_simple(line):
    """
    return: (video_id, features_array, label, chunk_nb, split_nb)
    """
    import re
    ...
    try:
        ...
        try:
            features = np.array([float(x.strip()) for x in data_str.split(',') if x.strip()], 
                               dtype=np.float32)
                               
            if features.size == 0:
                print(f"Empty feature: {data_str}")
                return None
        except Exception as e:
            print(f"Error when parsing feature: {str(e)}")
            print(f"The string is: '{data_str}'")
            return None
            
        return video_id, features, label, chunk_nb, split_nb

    except Exception as e:
        print(f"Error when parsing line: {str(e)}")
...
def merge(eval_path, num_tasks):
    ...
    for x in range(num_tasks):
        ...
        for line in lines:
            ...
            result = parse_line_simple(line)
            if result is None:
                print(f"Parsing line error: {line_id}")
                continue

            name, data, label, chunk_nb, split_nb = result
            ...
            pos_key = chunk_nb + split_nb
            if pos_key in dict_pos[name]:
                continue

            dict_feats[name].append(data)
            dict_pos[name].append(pos_key)

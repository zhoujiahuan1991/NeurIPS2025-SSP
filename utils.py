...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if loss_scaler is not None:
        ...
        if args.test_best and args.eval:
            try:
                load_specific_model(model, model_ema, args, output_dir, model_name='best', load_optimizer_states=False)
                flag = True
...
def load_specific_model(model, model_ema, args, output_dir, model_name, load_optimizer_states=True):
    args.resume = os.path.join(output_dir, f'checkpoint-{model_name}')
    print(f"Auto resume the {model_name} checkpoint")
    _, client_states = model.load_checkpoint(args.output_dir, tag=f'checkpoint-{model_name}', load_optimizer_states=load_optimizer_states)

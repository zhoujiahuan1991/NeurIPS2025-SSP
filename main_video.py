...
def get_args():
    ...
    parser.add_argument('--data_set', default='Kinetics', choices=[
        'Kinetics', 'Kinetics_sparse', 'diving48',
    ...
    parser.add_argument('--fix_seed', default=False, action='store_true', help='fix seed for multi-GPU training')
    ...
    parser.add_argument('--linear', default=False, action='store_true', help='whether use linear probing')

    parser.add_argument('--ours', default=False, action='store_true', help='whether use our method')
    parser.add_argument('--ours_num', default=3, type=int, help='number of our attention blocks')
    parser.add_argument('--ours_inner_dim', default=384, type=int, help='dim of our inner blocks')
    parser.add_argument('--ours_attn_dim', default=256, type=int, help='dim of our attention blocks')
...
def main(args, ds_init):
    ...
    tuning_config = EasyDict(
        # Ours related
        ours=args.ours,
        ours_num=args.ours_num,
        ours_inner_dim=args.ours_inner_dim,
        ours_attn_dim=args.ours_attn_dim
    )
    ...
    if args.finetune and not args.eval:
        ...
        utils.interpolate_pos_embed(model, checkpoint_model, args)

        # load pre-trained model
        missing_keys = utils.load_state_dict(model, checkpoint_model, prefix=args.model_prefix)
    ...
    if not args.resume:
        ...
        for _, p in model.head.named_parameters():
            p.requires_grad = True
        for _, p in model.norm_f.named_parameters():
            p.requires_grad = True
